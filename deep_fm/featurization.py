...
def dataprocess():
    ...
    for x in df.index:
        ...
        if df.loc[x, 'Price'] < 2000:
            df.loc[x, 'Price'] = 2000
        if df.loc[x, 'Price'] > 15000:
            df.loc[x, 'Price'] = 15000

        if df.loc[x, 'TotalPrice'] < 10:
            df.loc[x, 'TotalPrice'] = 10
        if df.loc[x, 'TotalPrice'] > 150:
            df.loc[x, 'TotalPrice'] = 150
    ########################### featurization ########################
    # 等频分箱
    # df["test"] = pd.qcut(df["BuildingSize"].rank(method="first"), 100)
    # d1 = df.groupby("test",as_index=True)
    # d2 = pd.DataFrame()
    # d2["min"] = d1.min().BuildingSize
    # #d2["max"] = d1.max().BuildingSize
    # #d2["total"] = d1.count().BuildingSize
    #
    # d2.plot()
    # plt.show()
    # 等距分箱
    # df["test"] = pd.cut(df["BuildingSize"].rank(method="first"), 100)
    # d1 = df.groupby(["test"],as_index=True)
    # d2 = pd.DataFrame()
    # d2["min"] = d1.min().BuildingSize
    # #d2["max"] = d1.max().BuildingSize
    ...
    d2 = pd.DataFrame()
    d2["sort"] = df["BuildingSize"].sort_values()
    d2 = d2.reset_index(drop=True)
    d3 = pd.DataFrame()
    d3["sort"] = pd.cut(d2["sort"], 25)
    d4 = pd.DataFrame()
    d4 = pd.value_counts(d3["sort"])
    d4.sort_index(inplace=True)
    print(str(d4.index[0]))
    ...
    #item_ = item_[torch.arange(1980)!=0]
    neg_item_ = torch.cat((item_[2:],item_[:2]),0)
    # print(item_.shape)
    # print(item_[100:105])
    ...
    #return user_, item_, neg_item_, user_scaler, item_scaler


    test_user_ = user_[:100]
    test_item_ = item_[:100]
    test_neg_item_ = []

    for i in range(len(test_user_)+1):
        temp = item_[i+100:i+200]
        # if i == 0:
        #     test_neg_item_ = temp
        # else:
        #     test_neg_item_ = torch.cat((test_neg_item_, temp),0)
        test_neg_item_.append(temp)


    return user_, item_, neg_item_, test_user_, test_item_, test_neg_item_
